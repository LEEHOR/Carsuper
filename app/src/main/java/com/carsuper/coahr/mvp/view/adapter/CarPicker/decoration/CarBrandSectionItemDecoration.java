 ...
 */
public class CarBrandSectionItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public CarBrandSectionItemDecoration(Context context, List<CarBrandBean.JdataEntity.BrandEntityX.BrandEntity> data) {
        this.mData = data;
    ...
    public void setData(List<CarBrandBean.JdataEntity.BrandEntityX.BrandEntity> data) {
        this.mData = data;
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mData != null && !mData.isEmpty() && position <= mData.size() - 1 && position > -1) {
                if (position == 0) {
                    ...
                    if (null != mData.get(position).getFirst_letter()
                            && !mData.get(position).getFirst_letter().equals(mData.get(position - 1).getFirst_letter())) {
                        drawSection(c, left, right, child, params, position);
                    }
                }
            }
    ...
    private void drawSection(Canvas c, int left, int right, View child,
        ...
        mTextPaint.getTextBounds(mData.get(position).getFirst_letter(),
                0,
                mData.get(position).getFirst_letter().length(),
                mBounds);
        c.drawText(mData.get(position).getFirst_letter(),
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        String section = mData.get(pos).getFirst_letter();
        View child = parent.findViewHolderForLayoutPosition(pos).itemView;

        boolean flag = false;
        if ((pos + 1) < mData.size()) {//不是处于最后一个position
            if (null != section && !section.equals(mData.get(pos + 1).getFirst_letter())) {//标题不为空且跟下一个标题 不同
